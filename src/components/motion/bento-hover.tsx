 ...
 * DOCUMENT-space rect to `bridge.hoverRect` — the lattice formation tightens
 * nearby particles toward the frame.
 *
 * Inert-safe (D-08): without a mounted canvas the writes are dead letters and
 * `invalidate` is a module-level no-op — excluded visitors pay two no-op event
 * handlers, nothing else. pointer:fine only (D-06): on touch every event is a
 * cheap early return, and the out-handler bails before its write so taps never
 * schedule a frame.
 */
